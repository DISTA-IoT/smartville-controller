...
def launch(**kwargs):
    ...
    the tiger component is run from pox using the following command:
    
    ```
    $ pox.py smartController.tiger 
    ```

    It migth be useful to launch some PoX logging components alonside the smartController:

    ```
    $ /pox/pox.py samples.pretty_log smartController.tiger 
    ```
    
    You can also add parameters for this component using the longparam syntax (i.e., double hyphens):

      ```
    $ /pox/pox.py samples.pretty_log smartController.tiger --use_packet_feats=false --multi_class=false --flow_idle_timeout=3
    ```
