...
CFG = dict(
    ...
    # The agent cannot afford all labels; it must choose which ones to buy.
    max_steps    = 150,
    init_budget  = 15.0,
    min_budget   = 0.0,
    max_budget   = 40.0,
    ...
    r_accept_A_uninformed =  -7.0,   # trap: A looks benign, acceptance is catastrophic
    r_block_A_uninformed  =   0.5,
    r_accept_B_uninformed =  -3.5,
    r_block_B_uninformed  =   1.5,
    r_accept_C_uninformed =   0.5,
    r_block_C_uninformed  =  -1.5,
    r_accept_D_uninformed =   0.5,
    ...
    # DAI-P: weight on epistemic gain in EFE target
...
class InferenceModule(nn.Module):
    ...
    Anomaly detection: distance to nearest known prototype > anomaly_threshold.
    Unknown clustering: temperature-softmax over distances to unk_protos.
    """
    ...
    def __init__(self, cfg: dict):
        ...
        self.known_types_list: List[str] = []

        self.register_buffer('unk_protos', torch.randn(n_u, d_h) * 0.1)
        self.register_buffer('unk_conf',   torch.zeros(n_u))   # per-cluster EMA confidence
    ...
    def known_classify(self, h: torch.Tensor):
        if h.dim() == 1:
            h = h.unsqueeze(0)
        dists = torch.cdist(h, self.known_protos)
        probs = F.softmax(-dists * self.proto_temp, dim=-1)
        return probs.argmax(-1), probs.max(-1).values, dists
    ...
    def unk_cluster(self, h: torch.Tensor):
        ...
        return weights.argmax(-1), weights.max(-1).values, weights
    ...
    def init_unk_protos_with_data(self, data_gen: DataGenerator, n_samples: int = 30):
        """
        Warm-start unknown cluster protos using the pretrained encoder.
        Avoids cold-start misassignment at episode start.
        """
...
#
# Four per-cluster confidences (EMA) replace the single scalar that was
# here before.  The transition network can now cleanly learn that buying
...
class BreakEvenAgent:
    """
    Deterministic heuristic: buy the unlabelled cluster with the highest
    anomaly score (distance to nearest known prototype) when affordable.

    Unknown-A has the LOWEST anomaly score (overlaps K0), so it is always
...
def run_episode(agent, env: SyntheticLIONEnv, train: bool = True) -> EpisodeStats:
    ...
    while True:
        ...
        if train:
            ...
            li = agent.train_step()
            if li:
                if 'loss'       in li: losses.append(li['loss'])
                if 'trans_loss' in li: trans_losses.append(li['trans_loss'])
...
def smooth(xs: List[float], w: int = 15) -> List[float]:
    ...
    for i, x in enumerate(xs):
        out.append(float(np.mean(xs[max(0, i - w): i + 1])))
    return out
