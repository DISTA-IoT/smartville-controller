...
"""
This model file is identical to default.py EXCEPT for ConfidenceDecoder, which
ports SIMBA's anomaly-detection rule (simba/inference.py): a sample is
anomalous iff its distance to the NEAREST known-class prototype exceeds a
threshold tau. See the ConfidenceDecoder docstring for the mapping.
"""
...
class ConfidenceDecoder(nn.Module):
    """
    SIMBA-style prototypical distance-threshold anomaly scorer.

    SIMBA's inference module (simba/inference.py) flags a sample as unknown
    iff its Euclidean distance to the nearest trainable-class prototype
    exceeds a threshold tau, and tau itself is *calibrated against the G1
    pseudo zero-days* (chosen as the best Known-vs-G1 separation). This
    decoder reproduces that decision rule inside TIGER's confidence-decoder
    interface:

      * `scores` arrives as the prototypical similarity slice the brain
        already computes -- scores = 1 / (euclidean distance + eps) to each
        known-class prototype -- so the nearest-prototype distance is
        recovered exactly as d_min = 1 / max_k(scores_k);
      * the anomaly probability is a sigmoid over how far d_min sits past a
        learnable threshold: p = sigmoid((d_min - tau) / softplus-scale).
        p > 0.5 <=> d_min > tau, so with ad_threshold = 0.5 the routing
        decision IS SIMBA's "unknown iff dist > tau" rule;
      * tau (and the sharpness scale) are learned by the same BCE/AD loss the
        other decoders use, whose targets label G1 samples anomalous and
        Known samples not -- i.e. tau is calibrated to separate Knowns from
        G1s, exactly what SIMBA's balanced-accuracy threshold search does,
        just via gradient descent; and because the loss also backprops into
        the encoder (ad_loss_backprop_to_encoder), G1 queries are pushed away
        from the Known prototypes, standing in for SIMBA's explicit
        ad_repulsion_margin term.

    Parameterised in log-space so tau and the scale stay strictly positive.
    Checkpoints from the parameter-free decoders (default/optim/mahalanobis)
    are empty; load_state_dict is non-strict so those still load, leaving
    tau/scale at their init values to be calibrated online.
    """

    def __init__(self, device):
        super(ConfidenceDecoder, self).__init__()
        self.device = device
        # exp(0) = 1.0: tau starts at distance 1 and the sigmoid at unit
        # sharpness; both are then calibrated by the AD/BCE loss.
        self.log_tau = nn.Parameter(torch.tensor(0.0))
        self.log_scale = nn.Parameter(torch.tensor(0.0))

    def forward(self, scores):
        # scores: [n_query, K] = 1 / (dist to each known prototype + 1e-10).
        # Nearest-prototype distance per query:
        d_min = 1.0 / (scores.max(dim=1, keepdim=True).values + 1e-10)
        tau = torch.exp(self.log_tau)
        scale = torch.exp(self.log_scale) + 1e-6
        return torch.sigmoid((d_min - tau) / scale)

    def load_state_dict(self, state_dict, strict=True):
        # Accept the empty state dicts saved by the parameter-free decoder
        # variants (and any checkpoint lacking log_tau/log_scale).
        return super(ConfidenceDecoder, self).load_state_dict(state_dict, strict=False)
