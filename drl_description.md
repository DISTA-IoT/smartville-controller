...
- **Known-traffic step** (`act_on_known_traffic`): the predicted-known online samples of the tick are grouped by the IM's predicted class (`torch.unique(class_preds)`), one DM decision *per predicted-class group*. The centroid is the mean hidden vector of that group's members; `cs_classif_confidence` is that group's own classification confidence, computed by `_cs_confidence_for_slice` over just that group's logits/predictions (same four `confidence_strategy` strategies as before — baseline/entropy/energy/margin — just scoped to the group instead of the whole tick). `zda_confidence` in this state slot still carries the tick-level anomaly-detection confidence (`self.zda_confidence`), since the anomaly/known split itself is a tick-wide property, not something that varies per closed-set group.
- **Unknown-cluster step** (`act_on_unknown_clusters`): unchanged in spirit — one DM decision *per identified cluster*, looped (`for idx, centroid in enumerate(centroids[~missing])`) — but `zda_confidence` is now that cluster's own confidence, computed by `_zda_confidence_for_subset` over just the anomaly probabilities of that cluster's members (cluster membership is read off `clusters_oh`'s column for the cluster, matched against the online samples' anomaly probabilities in the same order). `cs_classif_confidence` in this state slot still carries the tick-level closed-set confidence (`self.cs_classif_confidence`), for the same reason as above.

So the "state" the agent sees is no longer heterogeneous by construction: a same-shaped vector that is always "here's a specific group/cluster's centroid + scalars", whether the tick is processing known or unknown traffic, and whether it's group 1, 2, or N within that tick. The exteroceptive part only falls back to a `-1`-filled placeholder for the very last group/cluster of a tick's loop, as the "next state" when there is no next group/cluster to chain to (see §2.3) — never as a stand-in for "no information available", as it previously was for every known-traffic decision.
